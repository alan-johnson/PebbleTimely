...
#define LAYOUT_SLOT_TOP      24 // status bar height; each slot gets (device_height - LAYOUT_SLOT_TOP) / 2
// stat_batt_left computed at runtime: device_width - STAT_BATT_WIDTH - STAT_BATT_NIB_WIDTH - 1
...
static void handle_battery(BatteryChargeState charge_state) {
  ...
  layer_set_frame(inverter_layer_get_layer(battery_meter_layer), GRect(stat_batt_left+2, s_batt_top+2, battery_meter, s_batt_height-4));
...
static void window_unload(Window *window) {
  ...
  inverter_layer_destroy(battery_meter_layer);
...
static void init(void) {
  ...
  if (persist_exists(PK_SETTINGS)) {
    ...
    if (settings.version == 12) { // v12 -> v13: strftime_format changed from char* (4B) to char[32]
      // Old layout placed track_battery at byte 17 (= strftime_format[4] in new layout).
      // The 4-byte pointer that lived at strftime_format[0..3] is stale; discard it.
